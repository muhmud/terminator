...
class SqlMode(plugin.MenuItem):
    ...
    def psql_keypress(self, terminal, event):
        ...
        if (event.keyval == 65475 and event.state == 24):
            # ^[[17;3~
            terminal.vte.feed_child_binary(b'\x1b\x5b\x31\x37\x3b\x33\x7e')
            time.sleep(0.4)
            terminal.emit('print', 'next', '\n#q\n\\i /ram/buffer.sql\n')
        # Check for Alt + D
        if (event.keyval == 100 and event.state == 24):
            # ^[[24;9{
            terminal.vte.feed_child_binary(b'\x1b\x5b\x32\x34\x3b\x39\x7b')
            time.sleep(0.4)
            terminal.emit('print', 'next', '\n#q\n\\i /ram/buffer.sql\n')
        # Check for Alt + Shift + D
        if (event.keyval == 68 and event.state == 25):
            # ^[[24;9}
            terminal.vte.feed_child_binary(b'\x1b\x5b\x32\x34\x3b\x39\x7d')
            time.sleep(0.4)
            terminal.emit('print', 'next', '\n#q\n\\i /ram/buffer.sql\n')
        # Check for Alt + Space
