...
class Terminal(gtk.VBox):
    ...
    __gsignals__ = {
        ...
        'navigate': (gobject.SIGNAL_RUN_LAST, gobject.TYPE_NONE,
            (gobject.TYPE_STRING,)),
        'print': (gobject.SIGNAL_RUN_LAST, gobject.TYPE_NONE,
    ...
    def connect_signals(self):
        ...
        self.vte.connect('key-release-event', self.on_keyrelease)
    ...
    # XXXXX
    def on_copy_on_selection(self, widget):
        clip = gtk.Clipboard(selection="PRIMARY")
        txt = clip.wait_for_text().strip()
        if (txt != ''):
	...
	if self.keypress_callback is not None:
            self.keypress_callback(self, event)
        ...
        if mapping:
            if mapping not in ['close_window', 
                                       'full_screen', 
                                       'new_tab']:
    ...
    def on_keyrelease(self, widget, event):
        if (event.state & gtk.gdk.CONTROL_MASK == gtk.gdk.CONTROL_MASK and event.keyval == 65507):
            with open('/ram/emacs-ctrl-up', 'w') as the_file:
    ...
    def on_buttonpress(self, widget, event):
        ...
        if event.button == 1:
            ...
            # XXXXX
            # ctrl + rightclick brings up options
            if event.state & gtk.gdk.CONTROL_MASK == gtk.gdk.CONTROL_MASK:
                self.popup_menu(widget, event)
                return(True)
...
%s plugin' % urlplugin.handler_name)
    ...
    def key_tab_stack_up(self):
        self.emit('tab-change', -3)

    def key_tab_stack_down(self):
