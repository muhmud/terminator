...
class Terminator(Borg):
    ...
    def prepare_attributes(self):
        ...
        if not self.keybindings:
            ...
            self.keybindings.configure(self.config['keybindings'], self.config['keycodes'])
    ...
    # XXXXX
    def find_next_terminal_by_uuid(self, uuid):
        """Search our terminals for one matching the supplied UUID"""
        dbg('searching self.terminals for next after: %s' % uuid)
        next = False;
        for terminal in self.terminals:
            dbg('checking: %s (%s)' % (terminal.uuid.urn, terminal))
            if next == True:
