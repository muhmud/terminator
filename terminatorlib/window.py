...
class Window(Container, gtk.Window):
    ...
    def on_key_press(self, window, event):
        ...
        if (event.state == 16 and event.keyval == 65027):
            self.switching = True
    ...
    def on_key_release(self, window, event):
        if (event.state == 144 and event.keyval == 65027):
            maker = Factory()
            child = self.get_child()
            if maker.isinstance(child, 'Notebook'):
                child.tab_switched(self.switch_tab_pos)
            self.switch_tab_pos = 0
...
(fwidth:%s)(fheight:%s)' % (extra_width, extra_height, 
    ...
    def tab_change(self, widget, num=None):
        ...
        if num == -1:
            ...
            # Stack up
            if self.switch_tab_pos == 0:
                self.switch_tab_pos = child.get_n_pages() - 1
            else:
                self.switch_tab_pos -= 1
            num = child.get_tab(self.switch_tab_pos)
        elif num == -4:
            # Stack down
            if (self.switch_tab_pos == child.get_n_pages() - 1):
                self.switch_tab_pos = 0
            else:
                self.switch_tab_pos += 1
            num = child.get_tab(self.switch_tab_pos)
    ...
    def move_tab(self, widget, direction):
        ...
        notebook.fix_tab_stack(orig_page, page)

    def find_terminal(self, terminal, direction):
        """Find a relative terminal"""
...
class WindowTitle(object):
    ...
    def update(self):
        ...
        if not self.window.switching:
            ...
            if maker.isinstance(child, 'Notebook'):
                child.tab_switched_now(child.get_current_page())
